...
public class BoundedSetOfNaturals implements Iterable<Integer> {
    ...
    public void add(int[] numbers) {
        ...
        //check if the array is bigger than the maxSize
        if (numbers.length > maxSize || numbers.length + this.collection.size() > maxSize){
            throw new IllegalArgumentException("add array: bounded set does not have enough space.");
        }
    ...
    public boolean intersects(BoundedSetOfNaturals subset) {
        return this.collection.stream().anyMatch(subset.collection::contains);
    }
    ...
    public int count(Integer element) {
        ...
        return (int) collection.stream().filter(e -> e.equals(element)).count();
    }
